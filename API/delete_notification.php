...
if (!$notificationId || !$userId) {
    sendJsonResponse(false, "Missing required parameters: notification_id and user_id are required.", [], 400);
}

try {
    $stmt = $pdo->prepare("DELETE FROM notifications WHERE notification_id = :notification_id AND user_id = :user_id");
    ...
    if ($stmt->rowCount() > 0) {
        ...
        sendJsonResponse(false, "Notification not found.", [], 404);
    }
