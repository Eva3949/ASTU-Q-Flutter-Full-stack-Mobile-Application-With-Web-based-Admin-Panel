...
try {
    ...
    if (!$searchQuery) {
        sendJsonResponse(false, "Missing required parameter: query.", [], 400);
    }
    ...
    $sql .= " FROM questions q
            ...
            WHERE q.status = 'approved' AND (q.title LIKE :search_title OR q.content LIKE :search_content OR q.tags LIKE :search_tags)";
    ...
    $stmt->bindValue(':search_tags', $searchParam, PDO::PARAM_STR);
    ...
    $countSql = "SELECT COUNT(*) FROM questions q WHERE q.status = 'approved' AND (q.title LIKE :search_query OR q.content LIKE :search_query OR q.tags LIKE :search_query)";
    $countStmt = $pdo->prepare($countSql);
    $countStmt->bindParam(':search_query', $searchParam, PDO::PARAM_STR);
    $countStmt->execute();
