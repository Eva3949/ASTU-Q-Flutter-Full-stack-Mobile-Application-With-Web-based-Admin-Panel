...
try {
    ...
    $params = [];
    if ($viewerId) {
        $params[':viewer_id_up'] = $viewerId;
        $params[':viewer_id_down'] = $viewerId;
        $params[':viewer_id_book'] = $viewerId;
    }

    if ($subject) {
        $sql .= " AND q.subject = :subject";
        $params[':subject'] = $subject;
    }

    if ($search) {
        $sql .= " AND (q.title LIKE :search_title OR q.content LIKE :search_content)";
    ...
    foreach ($questions as &$question) {
        ...
        $question['is_resolved'] = $question['answer_count'] > 0; // Simple logic for now
    ...
    $countSql = "SELECT COUNT(*) FROM questions q WHERE q.status = 'approved'";
    $countParams = [];

    if ($subject) {
        $countSql .= " AND q.subject = :subject";
        $countParams[':subject'] = $subject;
    }

    if ($search) {
        $countSql .= " AND (q.title LIKE :search_title OR q.content LIKE :search_content)";
        $countParams[':search_title'] = '%' . $search . '%';
        $countParams[':search_content'] = '%' . $search . '%';
    }

    if ($userId) {
        $countSql .= " AND q.user_id = :user_id";
        $countParams[':user_id'] = $userId;
    }

    if ($featured) {
        $countSql .= " AND q.featured = 1";
    }
    ...
    sendJsonResponse(true, "Questions fetched successfully", [
        'questions' => $questions,
