...
try {
    ...
    $sql .= " FROM questions q
            ...
            WHERE q.featured = 1 AND q.status = 'approved'
    ...
    $countSql = "SELECT COUNT(*) FROM questions q WHERE q.featured = 1 AND q.status = 'approved'";
    $countStmt = $pdo->prepare($countSql);
    $countStmt->execute();
    $totalQuestions = $countStmt->fetchColumn();

    sendJsonResponse(true, "Featured questions fetched successfully.", [
        'questions' => $questions,
    ...
    error_log("Error fetching featured questions: " . $e->getMessage());
